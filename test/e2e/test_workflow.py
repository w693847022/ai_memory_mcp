...
def test_complete_workflow():
    ...
    try:
        ...
        tag_service.register_tag(project_id, "feature", "新功能")
        tag_service.register_tag(project_id, "backend", "后端模块")
        print("    ✓ 标签已注册")
        ...
        # 7. 更新功能状态
        print("  步骤7: 更新功能状态...")
        result = project_service.update_item(project_id, "features", feature_id, status="in_progress")
        ...
        # 11. 删除功能
        print("  步骤11: 删除功能...")
        result = project_service.delete_item(project_id, "features", feature_id)
        ...
        # 12. 重命名项目
        print("  步骤12: 重命名项目...")
        result = project_service.project_rename(project_id, "电商平台升级")
...
def test_multi_project_workflow():
    ...
    try:
        ...
        # 创建多个项目
        print("  创建多个项目...")
        project_a = project_service.register_project("项目A", "/path/a", tags=["web"])
        project_b = project_service.register_project("项目B", "/path/b", tags=["api"])
        project_c = project_service.register_project("项目C", "/path/c", tags=["mobile"])

        # 为每个项目添加内容（使用 add_item 统一接口）
        project_service.add_item(project_id=project_a["project_id"], group="features", content="Web前端功能内容", summary="Web前端功能", status="pending", tags=["web"])
        project_service.add_item(project_id=project_b["project_id"], group="features", content="API接口功能内容", summary="API接口功能", status="pending", tags=["api"])
        project_service.add_item(project_id=project_c["project_id"], group="features", content="移动端功能内容", summary="移动端功能", status="pending", tags=["mobile"])
        ...
        # 验证各项目数据独立
        data_a = project_service.get_project(project_a["project_id"])
        data_b = project_service.get_project(project_b["project_id"])
        data_c = project_service.get_project(project_c["project_id"])
