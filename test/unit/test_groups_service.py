...
def test_validate_tags_count_within_limit():
    ...
    # 测试2个标签（等于限制）
    is_valid, error = ItemValidator.validate_tags_count(["tag1", "tag2"], config)
    assert is_valid, f"2个标签应该通过验证: {error}"
    assert error is None, "错误消息应该为空"

    # 测试1个标签（少于限制）
    is_valid, error = ItemValidator.validate_tags_count(["tag1"], config)
...
def test_validate_tags_count_exceeds_limit():
    ...
    # 测试3个标签（超过限制）
    is_valid, error = ItemValidator.validate_tags_count(["tag1", "tag2", "tag3"], config)
    ...
    # 测试5个标签（远超限制）
    is_valid, error = ItemValidator.validate_tags_count(["tag1", "tag2", "tag3", "tag4", "tag5"], config)
...
def test_validate_tags_count_empty_list():
    ...
    # 测试空列表
    is_valid, error = ItemValidator.validate_tags_count([], config)
...
def test_validate_tags_count_custom_config():
    ...
    is_valid, error = ItemValidator.validate_tags_count(
...
def test_validate_tags_count_none_config():
    ...
    # 传入 None 配置
    is_valid, error = ItemValidator.validate_tags_count(["tag1"], None)
    assert is_valid, f"1个标签应该通过验证（None配置）: {error}"

    # None 配置应该使用默认值2
    is_valid, error = ItemValidator.validate_tags_count(["tag1", "tag2", "tag3"], None)
